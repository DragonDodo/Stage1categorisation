...
for iProc in range(nClasses):
    ...
    cutLo = np.arange(0,1.1,0.1).tolist()

    cutHi = np.arange(0,1.1,0.1).tolist()
    ...
    for i in cutLo:
        for j in cutHi:
            if j>=i:
                ...
                sigWeightsHi = diphoFW * (diphoY==1) * (diphoS-3==iProc) * (diphoPredY>j) * (diphoR==iProc)
                ...
                bkgWeightsHi = dataFW * (dataPredY>j) * (dataR==iProc)
                ...
                sigWeightsLo = diphoFW * (diphoY==1) * (diphoS-3==iProc) * (diphoPredY<j) * (diphoPredY>i) * (diphoR==iProc)
                ...
                bkgWeightsLo = dataFW * (dataPredY<j) * (dataPredY>i) * (dataR==iProc)
                ...
                sigvals.append(theSignif)
            else:
                sigvals.append(-1)
    print 'values for proc', iProc, sigvals
    ...
    p = plt.pcolormesh(X,Y,Z, vmin = 0)
    plt.axis([X.min(), X.max(), Y.min(), Y.max()])
    p.cmap.set_under('w')
    plt.colorbar()
    plt.xlabel('High cut value')
    plt.ylabel('Low cut value')
    plt.title('Heatmap of significance - proc %s'%(iProc))

    #plt.show() 
    
    plt.savefig('%s/heatmap%s.pdf'%(plotDir,iProc))
    plt.clf()  
